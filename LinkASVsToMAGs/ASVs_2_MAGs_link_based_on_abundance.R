...
#This scripts is trying to link MAGs with ASVs
# based on the correlation between ASVs-abundance and MAGs-abundance in many samples 
...
#regression plot x and y orders are the same with 't(spearman_res_r)'
#
...
while (i <= nrow_plot) {
  ...
  while(j <= ncol_plot) {
    ...
    plot(x=fit_data$V2, y=fit_data$V1,
         ...
         main = paste(MAG_name,"\n",ASV_name,sep = ""),cex.main=0.5, #show MAGs and ASVs ID
         xlab = "", ylab= "",
         xaxt = "n", yaxt = "n")
